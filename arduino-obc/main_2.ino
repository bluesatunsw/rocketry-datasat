// https://github.com/TheChapu/GY-91/blob/master/MPU9250_MS5637_AHRS_t3.ino
...
const int H3L_ADDRESS = 0x30; // H3LIS100DL I2C address 0x30 (SA0 GND) 0x32 (SA0 VCC)
...
//  Serial.println("Calibration values: ");
  ...
  // CTRL1 0x20 [0x67] 011 (ODR_LP=1Hz)  00 (ODR=40Hz) 111 (xyz=enabled)
  // CTRL2 0x21 [0x80] 1 (Boot refresh) 00 (High pass off) 0 (FDS=0) 0 (Hpen2=0) 0 (Hpen1=0) 00 (HPCF10=1Hz)
  // CTRL3 0x22 [0x40] 0 (IHL no int) 1 (PP_OD open drain CHECK) 0 (LIR2) 00 (I2_CFG10) 0 (LIR1) 00 (I1_CFG10)
  // CTRL4 0x23 [0x00] 0 (4 wire interface)
  // CTRL5 0x24 [0x00] 0 (sleep to wake disabled)
  writeByte(H3L_ADDRESS, 0x20, 0x80);
  delay(200);
  writeByte(H3L_ADDRESS, 0x21, 0x67);
  delay(200);
  writeByte(H3L_ADDRESS, 0x22, 0x40);
  delay(200);
  writeByte(H3L_ADDRESS, 0x23, 0x00);
  delay(200);
  writeByte(H3L_ADDRESS, 0x24, 0x00);
  delay(200);

  // INT1_CFG 0x30 [0x00] 00 (AOI idk) 000000 (disable interrupt request)
  // INT1_SRC 0x31 [0x00] 00
  // INT1_THS 0x32 [0x00] 0x00
  // INT1_DURATION 0x33 [0x00]
  // INT2_CFG 0x34 [0x00] 0
  // INT2_CFG 0x35 [0x00] 0
  writeByte(H3L_ADDRESS, 0x30, 0x00);
  delay(200);
  writeByte(H3L_ADDRESS, 0x31, 0x00);
  delay(200);
  writeByte(H3L_ADDRESS, 0x32, 0x00);
  delay(200);
  writeByte(H3L_ADDRESS, 0x33, 0x00);
  delay(200);
  writeByte(H3L_ADDRESS, 0x34, 0x00);
  delay(200);
  writeByte(H3L_ADDRESS, 0x35, 0x00);
  delay(200);
}
...
// Second order corrections
 ...
 //   roll  *= 180.0f / PI;
    ...
    // OUT_Y 0x2B
    // OUT_Z 0x2D
    OUT_X = readByte(H3L_ADDRESS, 0x29); // H3L (50g acc) 8 bits x axis
    OUT_Y = readByte(H3L_ADDRESS, 0x2B); // H3L (50g acc) 8 bits y axis
    OUT_Z = readByte(H3L_ADDRESS, 0x2D); // H3L (50g acc) 8 bits z axis
